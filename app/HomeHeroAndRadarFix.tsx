...
function mapById(institutions: Kg[], snapshot: PlaygroundSnapshot) { const map = new Map<string, Kg>(); const add = (kg: Kg) => { map.set(kg.id, kg); map.set(rawId(kg.id), kg); map.set(catalogId(kg.id), kg); }; institutions.forEach(add); snapshot.kindergartens.forEach(add); return map; }
function buildRadar(snapshot: PlaygroundSnapshot, institutions: Kg[]) {
  const prefs = readPrefs();
  const map = mapById(institutions, snapshot);
  ...
  active.forEach((request) => {
    const fromStat = ensure(request.from_kindergarten_id); const wantedId = wanted.get(request.id); const wantedStat = wantedId ? ensure(wantedId) : null;
