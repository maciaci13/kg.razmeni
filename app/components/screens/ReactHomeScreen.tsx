...
function FullScreenSheet({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  return <div className="coord-modal on" onClick={onClose}><div className="coord-modal-card" onClick={(event) => event.stopPropagation()}><button className="coord-close" type="button" onClick={onClose}>×</button>{children}</div></div>;
}
