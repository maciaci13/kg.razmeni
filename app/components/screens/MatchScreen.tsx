...
const currentStep = {
  index: 2,
  total: 6,
  title: "Отвори чата и пиши с другите родители",
};
...
const participants: Participant[] = [
  {
    ...
    avatar: "Ти",
    cardTone: "orange",
    route: "ДГ №25 Изворче → ДГ №25 Изворче – сграда 2",
    status: "Още не съм започнал/а",
    isMe: true,
  },
  {
    id: "parent-c",
    ...
    avatar: "1",
    cardTone: "green",
    route: "ДГ №25 Изворче – сграда 3 → ДГ №25 Изворче",
    status: "Още не съм започнал/а",
  },
  {
    id: "parent-b",
    ...
    avatar: "Б",
    cardTone: "purple",
    route: "ДГ №25 Изворче – сграда 2 → ДГ №25 Изворче – сграда 3",
    status: "Още не съм започнал/а",
  },
];
...
const mapNodes: MapNode[] = [
  {
    id: "sun",
    label: "ДГ 25 „Слънчеви лъчи“",
    color: "#FF7A22",
    dotClass: "left-[64px] top-[34px]",
    labelClass: "left-[14px] top-[110px]",
  },
  {
    id: "rainbow",
    label: "ДГ 30 „Дъга“ — филиал Изток",
    color: "#79A758",
    dotClass: "left-[226px] top-[144px]",
    labelClass: "right-0 top-[212px]",
  },
  {
    id: "bear",
    label: "ДГ 184 „Мечо Пух“ — ясла",
    color: "#8D66D7",
    dotClass: "left-[96px] top-[218px]",
    labelClass: "left-[54px] top-[282px]",
  },
];
      ...
      <div className="relative z-10 grid grid-cols-[1fr_126px] gap-2.5">
        <div>
          <div className="mb-3 flex h-[38px] w-max items-center justify-center rounded-full bg-gradient-to-br from-[#FF8A3D] to-[#C98555] px-4 font-display text-[13px] font-black tracking-[-0.02em] text-white shadow-[0_12px_24px_rgba(255,138,61,0.20)]">
            Стъпка: {currentStep.index}/{currentStep.total}
          </div>
...
function MiniStep({ top, label, tone }: { top: string; label: string; tone: "done" | "active" | "next" }) {
  if (tone === "done") {
    return (
      <div className={`absolute left-0 ${top} z-10 grid h-[22px] grid-cols-[22px_1fr] items-center gap-2 font-display text-[11px] font-extrabold leading-none tracking-[-0.02em] text-[#9AA486]`}>
        <span className="grid h-[22px] w-[22px] place-items-center rounded-full bg-[#DFE8CF] text-[#5F7D33]"><Check className="h-3 w-3" /></span>
  ...
  if (tone === "active") {
    return (
      <div className={`absolute left-0 ${top} z-10 grid h-[22px] grid-cols-[22px_1fr] items-center gap-2 font-display text-xs font-extrabold leading-none tracking-[-0.02em] text-foreground`}>
        <span className="h-[22px] w-[22px] rounded-full border-[5px] border-[#FF8A3D] bg-white shadow-[0_8px_18px_rgba(255,138,61,0.25)]" />
        <span>{label}</span>
      </div>
    );
  }
  ...
  return (
    <div className={`absolute left-0 ${top} z-10 grid h-[22px] grid-cols-[22px_1fr] items-center gap-2 font-display text-[11px] font-extrabold leading-none tracking-[-0.02em] text-[#A79A91] opacity-70`}>
      <span className="relative grid h-3 w-3 place-self-center rounded-full bg-white shadow-[0_0_0_10px_#fff] before:absolute before:inset-[-9px] before:z-[1] before:rounded-full before:bg-white after:absolute after:inset-[-6px] after:z-[2] after:rounded-full after:border-2 after:border-dashed after:border-[rgba(83,54,48,0.18)]" />
      <span>{label}</span>
    </div>
  );
}
  ...
  return (
    <div className={`relative overflow-hidden rounded-[26px] border border-black/5 bg-[#FBFAF6] ${compact ? "h-[360px]" : "h-[430px]"}`}>
      ...
      <div className={`absolute left-1/2 top-1/2 ${compact ? "h-[300px] w-[320px]" : "h-[430px] w-[360px]"} -translate-x-1/2 -translate-y-1/2`}>
        ...
        {mapNodes.map((node) => (
          <div key={node.id}>
            <div className={`absolute z-[3] ${node.dotClass}`}>
              <div className="grid h-[62px] w-[62px] place-items-center rounded-full bg-white shadow-[0_18px_34px_rgba(41,32,24,0.10)]">
                <span className="h-[26px] w-[26px] rounded-full border-[9px] border-white/70" style={{ background: node.color }} />
              </div>
            </div>
            <div className={`absolute z-[4] whitespace-nowrap rounded-full bg-white px-[13px] py-2 font-display text-[11px] font-black text-foreground shadow-[0_12px_24px_rgba(32,25,21,0.08)] ${node.labelClass}`}>
              {node.label}
            </div>
          </div>
        ))}
...
function ParticipantCard({ participant }: { participant: Participant }) {
  return (
    <section className={`mb-2.5 flex gap-3 rounded-[28px] border border-white/90 p-4 shadow-[0_14px_34px_rgba(80,54,35,0.09)] ${toneClass[participant.cardTone]}`}>
      <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl font-display text-sm font-black ${participant.isMe ? "bg-white/20" : "bg-white/45"}`}>
        {participant.avatar}
      </div>
      <div>
        ...
        {participant.isMe ? (
          <select className="mt-2.5 h-[31px] rounded-full border-0 bg-white/25 px-3.5 pr-7 font-display text-xs font-black text-white outline-none">
            <option>Още не съм започнал/а</option>
            <option>Пиша в чата</option>
            <option>Готов/а съм да продължа</option>
            <option>Имам въпрос</option>
          </select>
        ) : (
          <div className="mt-2.5 inline-flex h-[31px] items-center rounded-full bg-[#533630]/10 px-3.5 font-display text-xs font-black text-[#7B6E67]">
            {participant.status}
          </div>
        )}
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#211712]/20 p-[18px] backdrop-blur-[10px]" onClick={onClose}>
      <div className="relative max-h-[86vh] w-full max-w-[390px] overflow-hidden rounded-[32px] border border-white/90 bg-white/95 p-[18px] shadow-[0_24px_70px_rgba(33,23,18,0.20)]" onClick={(event) => event.stopPropagation()}>
        <button onClick={onClose} className="absolute right-4 top-4 grid h-[38px] w-[38px] place-items-center rounded-2xl bg-white font-display text-[22px] font-black text-[#5A4039]">×</button>
  ...
  return (
    <>
      ...
      <div className="mt-5 flex items-center justify-between">
        ...
        <button onClick={() => setShowMap(true)} className="border-0 bg-transparent p-0 font-display text-sm font-black text-[#8B5F47]">Виж още</button>
      </div>
