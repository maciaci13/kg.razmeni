...
export function MatchScreen({ participants = [], selectedProfileId = "", participantRoute, participantName, allConfirmed = false, matchIsClosed = false, confirmMatch, declineMatch, updateMyStatus, loading = false }: MatchScreenProps) {
  ...
  return (
    <>
      ...
      {showMap ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-transparent p-[18px] backdrop-blur-none" onClick={() => setShowMap(false)}><div className="relative w-full max-w-[390px] rounded-[32px] border border-white/90 bg-white/95 p-[18px] shadow-[0_24px_70px_rgba(33,23,18,0.20)]" onClick={(event) => event.stopPropagation()}><button onClick={() => setShowMap(false)} className="absolute right-4 top-4 grid h-[38px] w-[38px] place-items-center rounded-2xl bg-white font-display text-[22px] font-black text-[#5A4039]">×</button><h2 className="font-display text-2xl font-black tracking-[-0.04em]">Схема на процеса</h2><div className="mt-5 rounded-[26px] bg-[#FBFAF6] p-8 text-sm text-muted-foreground">Карта на процеса ще се визуализира тук.</div></div></div> : null}
    </>
  );
}