  ...
  return (
    <>
      <div className="app-head">
        <div className="app-head-left"><div className="hello">Намери<br /><span>размяна</span></div><div className="sub">Координирай се с родители за преместване по официалния ред.</div></div>
        <div className="avatar" style={{ cursor: "pointer" }}>🔔</div>
      </div>
      <div className="card hero-card"><h2>Място за<br />Място</h2><div className="hero-stats"><button className="hero-stat" type="button" onClick={() => setTab("requests")}><div className="hero-stat-num" style={{ background: "#cfe6ef", color: "var(--ink)" }}>00</div><div className="hero-stat-lbl">Активни заявки<span className="hero-stat-arrow">›</span></div></button><button className="hero-stat" type="button" onClick={() => setTab("matches")}><div className="hero-stat-num" style={{ background: "#f4e4b8", color: "var(--ink)" }}>00</div><div className="hero-stat-lbl">Съвпадения<span className="hero-stat-arrow">›</span></div></button></div><div className="orb"></div></div>
      <div style={{ marginBottom: 14 }}><button className="btn primary" style={{ width: "100%", height: 54, fontSize: 15, fontWeight: 800 }} type="button" onClick={() => setPopup("radar")}>📡 Радар</button></div>
      ...
      <div className="glass-card" style={{ padding: 16 }}><div className="eyebrow orange" style={{ marginBottom: 10 }}>Какво да направя?</div><div style={{ display: "flex", flexDirection: "column", gap: 12 }}><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><div className="place-icon" style={{ width: 36, height: 36, borderRadius: 13, fontSize: 14, flexShrink: 0 }}>01</div><div><div className="place-title">Свържи се</div><div className="place-sub">Провери условията за преместване в конкретното учебно заведение.</div></div></div><div style={{ height: 1, background: "rgba(83,54,48,.07)" }}></div><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><div className="place-icon" style={{ width: 36, height: 36, borderRadius: 13, fontSize: 14, flexShrink: 0 }}>02</div><div><div className="place-title">Подай документи</div><div className="place-sub">Подай необходимите документи в конкретното заведение.</div></div></div></div></div>
      <button className="request-card" style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", width: "100%", textAlign: "left" }} type="button" onClick={() => setPopup("share")}><div style={{ width: 44, height: 44, borderRadius: 17, background: "linear-gradient(135deg,var(--orange),var(--brown2))", display: "grid", placeItems: "center", fontSize: 20, boxShadow: "0 10px 22px rgba(255,138,61,.25)", flexShrink: 0 }}>↗</div><div style={{ flex: 1 }}><div className="place-title">По-бързо съвпадение</div><div className="place-sub">Увеличи шанса — покани родители</div></div><span style={{ color: "var(--soft)", fontSize: 18 }}>›</span></button>
...
function FullScreenSheet({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  return <div className="coord-modal on" onClick={onClose}><div className="coord-modal-card" onClick={(event) => event.stopPropagation()}><button className="coord-close" onClick={onClose}>×</button>{children}</div></div>;
}

function RadarPopup({ onClose, setTab }: { onClose: () => void; setTab: (tab: AppTab) => void }) {
  const cards = [{ c: "card-pink", n: "ДГ №25 Изворче", a: "Банкя · гр. Банкя, ул. П. Д. Петков, №15", s: [0, 0, 0] }, { c: "card-teal", n: "ДГ №25 Изворче – сграда 2", a: "Банкя · гр. Банкя, ул. Восток–2, №4", s: [0, 0, 0] }, { c: "card-sand", n: "ДГ №25 Изворче – сграда 3", a: "Банкя · гр. Банкя, ул. Царибродска, №5", s: [0, 0, 0] }];
  return <FullScreenSheet onClose={onClose}><div className="eyebrow orange">Радар за шанс</div><div className="hello" style={{ marginBottom: 8 }}>Където има<br /><span>движение?</span></div><div className="sub" style={{ marginBottom: 16 }}>Показваме места с активни заявки и потенциал за бъдещо съвпадение.</div><div className="chip-row" style={{ marginBottom: 14 }}><button className="chip-sel on">Около мен</button><button className="chip-sel">Най-голям шанс</button><button className="chip-sel">Всички</button></div><div className="search-bar"><span>🔍</span><span>Търси по име, номер или район...</span></div>{cards.map((card) => <div key={card.n} className={`radar-card ${card.c}`}><div className="radar-card-name">{card.n}</div><div className="radar-card-addr">{card.a}</div><div className="stat-row"><div className="stat-cell"><div className="stat-num">{card.s[0]}</div><div className="stat-label">родители я търсят</div></div><div className="stat-cell"><div className="stat-num">{card.s[1]}</div><div className="stat-label">може да я освободят</div></div><div className="stat-cell"><div className="stat-num">{card.s[2]}</div><div className="stat-label">Активни за твоя набор</div></div></div><button className="btn primary" style={{ height: 46, fontSize: 13 }} onClick={() => { onClose(); setTab("requests"); }}>Заяви размяна</button></div>)}</FullScreenSheet>;
}

function StepsPopup({ onClose }: { onClose: () => void }) {
  return <FullScreenSheet onClose={onClose}><div className="eyebrow orange">Първи стъпки</div><div className="hello" style={{ marginBottom: 14 }}>Как работи<br />процесът?</div>{["Настрой заявката", "Чакай системата", "Потвърди интерес", "Координирай се в чат", "Следвай официалния ред"].map((title, i) => <div key={title} className={`request-card ${i % 3 === 0 ? "green" : i % 3 === 1 ? "teal" : "sand"}`}><div className="row"><div className="place-icon">{String(i + 1).padStart(2, "0")}</div><div className="grow"><div className="place-title">{title}</div><div className="place-sub">Стъпката се изпълнява само през официалните правила на учебното заведение.</div></div></div></div>)}</FullScreenSheet>;
}
