...
export default function PlaygroundPage() {
  ...
  }, [snapshot, activeMatch, selectedProfileId, allConfirmed, matchIsClosed]);
  ...
  return (
    ...
    </div>{showLeaveOptions && activeMatch && !matchIsClosed ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/45 px-4 pb-4 backdrop-blur-sm" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-[2rem] bg-white p-5 shadow-soft"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-ink/40">Отказ от процеса</p><h2 className="mt-2 text-2xl font-black tracking-[-0.05em]">Как да затворим цикъла?</h2></div><button onClick={() => setShowLeaveOptions(false)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-paper text-xl font-black">×</button></div><p className="mt-3 text-sm font-medium leading-6 text-ink/55">Избери дали чатът да остане видим за координация след отказа или всичко да се затвори напълно.</p><div className="mt-5 grid gap-2"><button disabled={loading} onClick={() => leaveProcess(true)} className="rounded-full bg-ink px-4 py-4 text-sm font-black text-white disabled:opacity-40">Отказвам се, но запази чата</button><button disabled={loading} onClick={() => leaveProcess(false)} className="rounded-full bg-orange px-4 py-4 text-sm font-black text-white disabled:opacity-40">Отказвам се и затвори чата</button><button disabled={loading} onClick={() => setShowLeaveOptions(false)} className="rounded-full bg-beige px-4 py-4 text-sm font-black disabled:opacity-40">Връщам се назад</button></div></div></div> : null}</main>
  );
}
