...
function tone(p: Participant, allConfirmed: boolean) {
  ...
  if (p.confirmation_status === "pending") return "bg-white/80 text-ink/60";
  if (!allConfirmed || p.coordination_status === "can_continue") return "bg-lime text-ink";
  ...
  return "bg-white/80 text-ink/60";
}
...
export default function PlaygroundPage() {
  ...
  return (
    <main className="min-h-screen bg-paper px-4 py-5 text-ink"><div className="mx-auto max-w-md space-y-4">
      ...
      <section className="rounded-[2rem] bg-milk p-5 shadow-soft"><p className="text-xs font-bold uppercase tracking-[0.2em] text-ink/40">2. Профил</p><select value={selectedProfileId} onChange={(e) => setSelectedUserId(e.target.value)} className="mt-3 w-full rounded-2xl bg-paper px-4 py-4 text-sm font-black outline-none">{scenarioUsers.map((u) => <option key={u.id} value={u.id}>{u.display_name}</option>)}</select><p className="mt-3 text-xs text-ink/45">ID: {shortId(selectedProfileId)}</p></section>

      {!activeMatch ? <section className="rounded-[2rem] bg-milk p-5 shadow-soft"><h2 className="text-xl font-black tracking-[-0.04em]">Пусни заявка за размяна</h2><p className="mt-2 text-sm text-ink/55">Когато се появи цикъл, тази форма се прибира.</p><div className="mt-4 space-y-3"><label className="block text-xs font-bold uppercase tracking-[0.18em] text-ink/40">Имаме място в</label><select value={fromKgId} onChange={(e) => setFromKgId(e.target.value)} className="w-full rounded-2xl bg-paper px-4 py-4 text-sm font-bold outline-none"><option value="">Избери градина</option>{(snapshot?.kindergartens ?? []).map((kg) => <option key={kg.id} value={kg.id}>{kg.name} · {kg.district}</option>)}</select><label className="block text-xs font-bold uppercase tracking-[0.18em] text-ink/40">Желана градина</label><select value={wantedKgId} onChange={(e) => setWantedKgId(e.target.value)} className="w-full rounded-2xl bg-paper px-4 py-4 text-sm font-bold outline-none"><option value="">Избери желана градина</option>{(snapshot?.kindergartens ?? []).map((kg) => <option key={kg.id} value={kg.id}>{kg.name} · {kg.district}</option>)}</select><input value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)} className="w-full rounded-2xl bg-paper px-4 py-4 text-sm font-bold outline-none" /><div className="grid grid-cols-2 gap-2"><button disabled={loading || !selectedProfileId} onClick={fillDemo} className="rounded-full bg-beige px-5 py-4 text-xs font-black">Демо</button><button disabled={loading || !selectedProfileId} onClick={createRequest} className="rounded-full bg-ink px-5 py-4 text-xs font-black text-white disabled:opacity-40">Добави заявка</button></div></div></section> : null}
      {!activeMatch && myRequests.length ? <section className="rounded-[2rem] bg-milk p-5 shadow-soft"><h2 className="text-xl font-black tracking-[-0.04em]">Моите активни заявки</h2><div className="mt-4 space-y-3">{myRequests.map((r) => <div key={r.id} className="rounded-3xl bg-paper p-4"><p className="font-black">{kgById.get(r.from_kindergarten_id)?.name}</p><p className="mt-1 text-xs text-ink/55">Набор {r.child_group_year_or_age_group} · {r.is_active ? "Активна" : "Неактивна"}</p><div className="mt-3 grid grid-cols-2 gap-2"><button disabled={loading || !r.is_active} onClick={() => run({ action: "deactivateRequest", requestId: r.id })} className="rounded-full bg-beige px-3 py-3 text-xs font-bold disabled:opacity-30">Деактивирай</button><button disabled={loading || r.is_locked} onClick={() => run({ action: "deleteRequest", requestId: r.id })} className="rounded-full bg-ink px-3 py-3 text-xs font-bold text-white disabled:opacity-30">Изтрий</button></div></div>)}</div></section> : null}
...
function CycleMap({ participants, selectedProfileId, userById, fromToText, allConfirmed, loading = false, updateMyStatus, readOnly = false }: { participants: Participant[]; selectedProfileId: string; userById: Map<string, PlaygroundSnapshot["users"][number]>; fromToText: (p: Participant) => string; allConfirmed: boolean; loading?: boolean; updateMyStatus?: (status: string) => void; readOnly?: boolean }) {
  ...
  return <div className="mt-5 rounded-[1.75rem] bg-paper p-4"><p className="mb-4 text-xs font-black uppercase tracking-[0.2em] text-ink/40">Верига и статуси</p><div className="space-y-3">{participants.map((p, i) => { const me = p.user_id === selectedProfileId; const user = userById.get(p.user_id); return <div key={p.id} className="relative">{i < participants.length - 1 ? <div className="absolute left-8 top-[4.25rem] h-5 w-px bg-ink/20" /> : null}<div className={`relative z-10 rounded-[1.4rem] p-3 ${me ? "bg-ink text-white" : colors[i % colors.length]}`}><div className="flex items-center gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-white text-sm font-black text-ink shadow-sm">{me ? "Ти" : i + 1}</div><div className="min-w-0 flex-1"><p className="font-black leading-tight">{me ? "Ти" : user?.display_name}</p><p className={`mt-1 text-[11px] font-semibold leading-4 ${me ? "text-white/65" : "text-ink/60"}`}>{fromToText(p)}</p></div><span className={`shrink-0 rounded-full px-3 py-2 text-[10px] font-black ${tone(p, allConfirmed)}`}>{participantStatus(p, allConfirmed)}</span></div><div className="mt-3">{me && allConfirmed && !readOnly ? <select value={p.coordination_status} disabled={loading} onChange={(e) => updateMyStatus?.(e.target.value)} className="w-full rounded-2xl bg-white px-4 py-3 text-xs font-black text-ink outline-none disabled:opacity-50">{statusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : <div className={`rounded-2xl px-4 py-3 text-xs font-bold ${me ? "bg-white/10 text-white/70" : "bg-white/45 text-ink/65"}`}>{participantStatus(p, allConfirmed)}</div>}</div></div></div>; })}</div></div>;
}

function Timeline({ currentStep }: { currentStep: number }) {
  return <div className="mt-5 rounded-[1.75rem] bg-paper px-4 py-5">{steps.map((s, i) => { const n = i + 1; const done = n < currentStep; const current = n === currentStep; return <div key={s.title} className="relative grid grid-cols-[2rem_1fr] gap-3 pb-6 last:pb-0">{i < steps.length - 1 ? <div className={`absolute left-[0.94rem] top-8 h-[calc(100%-1.75rem)] w-px ${done ? "bg-lime" : "bg-ink/12"}`} /> : null}<div className={`relative z-10 grid h-8 w-8 place-items-center rounded-full text-xs font-black ${done ? "bg-lime text-ink" : current ? "bg-ink text-white" : "bg-ink/8 text-ink/35"}`}>{done ? "✓" : n}</div><div className="pt-0.5"><p className={`text-[10px] font-black uppercase tracking-[0.18em] ${current ? "text-ink" : "text-ink/40"}`}>Стъпка {n}</p><p className={`mt-1 text-sm font-black ${current ? "text-ink" : "text-ink/65"}`}>{s.title}</p><p className="mt-1 text-xs font-medium leading-5 text-ink/45">{s.helper}</p></div></div>; })}</div>;
}
