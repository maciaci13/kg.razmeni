...
function injectStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    .mzm-profile-clickable-row {
      cursor: pointer !important;
      -webkit-tap-highlight-color: transparent !important;
      transition: transform .14s ease, box-shadow .14s ease !important;
    }
    .mzm-profile-clickable-row:active {
      transform: scale(.985) !important;
    }
    .mzm-profile-subscreen {
      width: min(100%, 28rem) !important;
      margin-inline: auto !important;
      padding-bottom: calc(6rem + env(safe-area-inset-bottom, 0px)) !important;
    }
    .mzm-profile-subscreen-head {
    ...
    .mzm-profile-card + .mzm-profile-card {
    ...
    .mzm-profile-label {
      ...
      color: rgba(28,27,25,.42) !important;
    ...
    .mzm-profile-select-wrap {
      position: relative !important;
    }
    .mzm-profile-select-wrap::after {
      content: '⌄' !important;
      position: absolute !important;
      right: 1rem !important;
      top: 50% !important;
      transform: translateY(-52%) !important;
      color: rgba(28,27,25,.55) !important;
    ...
    .mzm-profile-save {
      ...
      font-size: .95rem !important;
      font-weight: 900 !important;
      box-shadow: 0 16px 34px rgba(249,94,8,.22) !important;
    }
    .mzm-profile-premium-hero {
      position: relative !important;
      overflow: hidden !important;
      border-radius: 2.2rem !important;
      background: linear-gradient(145deg, rgba(255,240,227,.98), rgba(236,237,199,.9)) !important;
      padding: 1.35rem !important;
      box-shadow: 0 22px 60px rgba(28,27,25,.1) !important;
    }
    .mzm-profile-premium-hero::after {
      content: '' !important;
      position: absolute !important;
      ...
      border-radius: 999px !important;
      background: rgba(217,231,203,.85) !important;
      pointer-events: none !important;
    }
    .mzm-profile-premium-hero > * {
      position: relative !important;
    ...
    .mzm-profile-premium-actions {
      display: grid !important;
      gap: .7rem !important;
      margin-top: 1rem !important;
    }
    .mzm-profile-premium-primary,
    .mzm-profile-premium-secondary {
      width: 100% !important;
      min-height: 3.55rem !important;
      border: 0 !important;
      border-radius: 999px !important;
    ...
    .mzm-profile-toast {
      ...
      border-radius: 999px !important;
      background: rgba(28,27,25,.82) !important;
      color: #fff !important;
      padding: .8rem 1rem !important;
      font-size: .85rem !important;
      font-weight: 800 !important;
      box-shadow: 0 18px 42px rgba(28,27,25,.22) !important;
    }
  `;
...
function openSubscreen(title: string, kicker: string, bodyHtml: string) {
  ...
  screen.innerHTML = `
    <div class="mzm-profile-subscreen-head">
      ...
      <div>
        <p class="mzm-profile-kicker">${escapeHtml(kicker)}</p>
        <h1>${title}</h1>
      </div>
    </div>
    ${bodyHtml}
...
function selectMarkup(name: string, label: string, options: { value: string; label: string }[], value = "") {
  return `
    <label>
      <span class="mzm-profile-label">${escapeHtml(label)}</span>
      <span class="mzm-profile-select-wrap">
        <select class="mzm-profile-select" name="${escapeHtml(name)}">
...
async function openProfileDataScreen() {
  ...
  const body = `
    <section class="mzm-profile-card">
      <p class="mzm-profile-copy" style="margin-top:0!important">Тези данни помагат на системата да предлага по-точни потенциални съвпадения. Можеш да ги редактираш по всяко време.</p>
      <form class="mzm-profile-form" data-mzm-profile-form style="margin-top:1rem!important">
        ...
        <button type="submit" class="mzm-profile-save">Запази данните</button>
      </form>
    </section>
  `;
  ...
  document.querySelector<HTMLFormElement>("[data-mzm-profile-form]")?.addEventListener("submit", (event) => {
...
function openPrivacyScreen() {
  openSubscreen("Поверителност", "Сигурност", `
    <section class="mzm-profile-card">
      <p class="mzm-profile-copy" style="margin-top:0!important">Място За Място е създадено така, че да показва само информацията, нужна за потенциална координация между родители — не повече.</p>
      <div class="mzm-profile-list">
        <div class="mzm-profile-list-item">Другите родители не виждат личните ти данни, документи или чувствителна информация.</div>
        <div class="mzm-profile-list-item">Заявката се използва само за търсене на потенциален маршрут между съвместими места.</div>
        <div class="mzm-profile-list-item">Чатовете се отключват само когато всички страни в потенциалния цикъл потвърдят интерес.</div>
        <div class="mzm-profile-list-item">Не публикувай ЕГН, медицински документи, адреси или снимки на деца в чата.</div>
      </div>
    </section>
  `);
}
...
function openRulesScreen() {
  openSubscreen("Правила и безопасност", "Важно", `
    <section class="mzm-profile-card">
      <p class="mzm-profile-copy" style="margin-top:0!important">Платформата помага на родителите да се открият и координират. Тя не е официална общинска система и не гарантира прием, преместване или размяна.</p>
      <div class="mzm-profile-list">
        <div class="mzm-profile-list-item">Не се продават места. Не се договарят плащания, услуги или обещания за прием.</div>
        <div class="mzm-profile-list-item">Всяка реална промяна трябва да мине по официалния ред и през съответните институции.</div>
        <div class="mzm-profile-list-item">Потвърждението в app-а означава само интерес за координация, не административно одобрение.</div>
        <div class="mzm-profile-list-item">Ако нещо изглежда съмнително, прекрати разговора и не споделяй допълнителни данни.</div>
      </div>
    </section>
  `);
}
...
function openSupportScreen() {
  openSubscreen("Подкрепи проекта", "Каузата", `
    <section class="mzm-profile-premium-hero">
      <p class="mzm-profile-copy" style="margin-top:0!important;color:rgba(28,27,25,.68)!important">Място За Място е малък независим проект с голяма цел: повече родители да намират път един към друг, без шум, хаос и безкрайно ровене по групите.</p>
      <div class="mzm-profile-list">
        <div class="mzm-profile-list-item">Помагаш да поддържаме каталога, търсенето и match логиката актуални.</div>
        <div class="mzm-profile-list-item">Помагаш проектът да остане независим, човешки и без агресивни реклами.</div>
        <div class="mzm-profile-list-item">Колкото повече родители се включат, толкова по-голям шанс има за реални съвпадения.</div>
      </div>
      <div class="mzm-profile-premium-actions">
        <button type="button" class="mzm-profile-premium-primary" data-mzm-open-share>Сподели с родители</button>
        <button type="button" class="mzm-profile-premium-secondary" data-mzm-support-interest>Искам да подкрепя финансово</button>
      </div>
    </section>
    <section class="mzm-profile-card">
      <p class="mzm-profile-kicker">Founding support</p>
      <p class="mzm-profile-copy">Скоро ще добавим лесен начин за малка подкрепа — като “кафе за проекта”. Не купуваш предимство в системата, а помагаш тя да съществува.</p>
    </section>
  `);

  document.querySelector<HTMLButtonElement>("[data-mzm-support-interest]")?.addEventListener("click", () => {
    showToast("Ще добавим опция за подкрепа скоро.");
  });
}
...
function bindProfileRows() {
  ...
  rows.forEach((row) => {
    ...
    if (text.includes("Данни за детето")) {
      row.innerHTML = row.innerHTML.replace("Данни за детето", "Данни за профила").replace("Набор, район и тип място", "Район, набор, тип място и сегашна градина");
    }
